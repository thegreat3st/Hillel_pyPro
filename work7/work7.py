...
class multi:
    ...
    def download_image(self):
        print(
            f"Downloading image from {self._image_url} in thread {threading.current_thread().name}"
        )
    ...
    def download_counter(self):
        ...
        worker = Thread(target=multi.download_image(self))
        worker.daemon = True
        worker.start()
...
def main():
    try:
        ...
        dict = {
            "Time taken for encryption task": mul.encryption_counter(),
